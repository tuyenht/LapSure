...
void CollectPlatformForensics(AuditReport&r,const FactoryProfile&p,const Capabilities&c,const std::wstring&,const std::atomic_bool* cancel){
 ...
 auto ev=RunProcessCapture(L"powershell.exe -NoProfile -NonInteractive -Command \"$s=(Get-Date).AddDays(-30);$ps=@('WHEA-Logger','Disk','stornvme','Ntfs','Display','Microsoft-Windows-Kernel-Power','BugCheck');foreach($p in $ps){try{$c=@(Get-WinEvent -FilterHashtable @{LogName='System';ProviderName=$p;StartTime=$s} -ErrorAction Stop).Count}catch{$c=0};'{0}|{1}' -f $p,$c}\"",25000,cancel);
 if(ev.launched&&!ev.timedOut&&!ev.output.empty()){for(auto&line:SplitLines(ev.output)){auto q=Split(line,L'|');if(q.size()<2)continue;auto n=ParseI64(q[1],0);if(q[0]==L"WHEA-Logger")r.hardware.events.whea=n;else if(q[0]==L"Disk")r.hardware.events.disk=n;else if(q[0]==L"stornvme")r.hardware.events.stornvme=n;else if(q[0]==L"Ntfs")r.hardware.events.ntfs=n;else if(q[0]==L"Display")r.hardware.events.display=n;else if(q[0]==L"Microsoft-Windows-Kernel-Power")r.hardware.events.kernelPower=n;else if(q[0]==L"BugCheck")r.hardware.events.bugCheck=n;}
   std::wstringstream x;x<<L"WHEA "<<r.hardware.events.whea<<L" | Disk "<<r.hardware.events.disk<<L" | NVMe "<<r.hardware.events.stornvme<<L" | NTFS "<<r.hardware.events.ntfs<<L" | Display "<<r.hardware.events.display<<L" | KernelPower "<<r.hardware.events.kernelPower<<L" | BugCheck "<<r.hardware.events.bugCheck;
   ...
   Add(r,L"Forensics",L"Historical hardware events / 30d",x.str(),L"0 preferred",st,Severity::Major,Dimension::Evidence,L"Historical evidence only; not a current-failure verdict");
 }
}
}
