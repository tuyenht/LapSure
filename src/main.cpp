...
const wchar_t* UiState(State s){switch(s){case State::Pass:return L"ĐẠT";case State::Good:return L"TỐT";case State::Warning:return L"CẦN LƯU Ý";case State::Fail:return L"KHÔNG ĐẠT";case State::Changed:return L"CÓ THAY ĐỔI";case State::NotTested:return L"CHƯA KIỂM TRA";case State::Unsupported:return L"KHÔNG HỖ TRỢ";default:return L"THÔNG TIN";}}
const wchar_t* UiDimension(Dimension d){switch(d){case Dimension::Identity:return L"Nhận diện";case Dimension::Factory:return L"Cấu hình gốc";case Dimension::Health:return L"Sức khỏe";case Dimension::Usage:return L"Mức sử dụng";case Dimension::Performance:return L"Hiệu năng";case Dimension::Stability:return L"Độ ổn định";case Dimension::Functional:return L"Chức năng";default:return L"Bằng chứng";}}
...
bool CanRunManualTest(HWND h){
    if(gRunning){MessageBoxW(h,L"LapSure đang kiểm tra tự động. Vui lòng chờ hoàn tất trước khi kiểm tra tương tác.",L"LapSure",MB_OK|MB_ICONINFORMATION);return false;}
    if(!gAuditReady){MessageBoxW(h,L"Hãy bấm “BẮT ĐẦU KIỂM TRA” trước. Kết quả tương tác sẽ được lưu chung vào báo cáo hiện tại.",L"LapSure",MB_OK|MB_ICONINFORMATION);return false;}
    return true;
}
...
int sel=(int)SendMessageW(gMode,CB_GETCURSEL,0,0);gSelectedMode=sel==2?L"Deep":(sel==1?L"Standard":L"Quick");
gRunning=true;gCancel=false;gAuditReady=false;EnableWindow(gOpen,FALSE);SetFunctionalButtonsEnabled(FALSE);SetWindowTextW(gBtn,L"DỪNG KIỂM TRA");SetWindowTextW(gStatus,L"Đang chuẩn bị kiểm tra...");gWorker=std::thread(AuditWorker,h);}
}
...
case WM_CREATE:{
 ...
 gOpen=CreateWindowW(L"BUTTON",L"XEM KẾT QUẢ",WS_CHILD|WS_VISIBLE,442,82,120,48,h,(HMENU)2,nullptr,nullptr);EnableWindow(gOpen,FALSE);
 gFuncDisplay=CreateWindowW(L"BUTTON",L"Màn hình",WS_CHILD|WS_VISIBLE,580,82,84,30,h,(HMENU)1201,nullptr,nullptr);EnableWindow(gFuncDisplay,FALSE);
 gFuncKeyboard=CreateWindowW(L"BUTTON",L"Bàn phím",WS_CHILD|WS_VISIBLE,670,82,84,30,h,(HMENU)1202,nullptr,nullptr);EnableWindow(gFuncKeyboard,FALSE);
...
case WM_AUDIT_DONE:{Fill();SetWindowTextW(gBtn,L"BẮT ĐẦU KIỂM TRA");EnableWindow(gOpen,!gReportPath.empty());SetFunctionalButtonsEnabled(w?FALSE:TRUE);if(gNext)EnableWindow(gNext,w?FALSE:TRUE);if(w)SetWindowTextW(gStatus,L"Đã dừng. Kết quả chưa đầy đủ và chưa thể kết luận.");else SetWindowTextW(gStatus,(L"Đã xong phần tự động — "+std::to_wstring(gReport.findings.size())+L" hạng mục. Hãy bấm TIẾP TỤC BƯỚC KẾ để hoàn tất.").c_str());InvalidateRect(h,nullptr,TRUE);if(gCloseRequested){DestroyWindow(h);}return 0;}
...
case WM_PAINT:{PAINTSTRUCT p;HDC dc=BeginPaint(h,&p);SetBkMode(dc,TRANSPARENT);SetTextColor(dc,text);HFONT h1=CreateFontW(32,0,0,0,FW_SEMIBOLD,0,0,0,DEFAULT_CHARSET,0,0,CLEARTYPE_QUALITY,0,L"Segoe UI");auto old=SelectObject(dc,h1);TextOutW(dc,28,24,L"LapSure",7);SelectObject(dc,old);DeleteObject(h1);SetTextColor(dc,RGB(90,100,115));const wchar_t*sub=L"Kiểm tra toàn diện laptop — dễ hiểu, minh bạch và có bằng chứng";TextOutW(dc,30,65,sub,(int)wcslen(sub));EndPaint(h,&p);return 0;}
