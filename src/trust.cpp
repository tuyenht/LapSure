...
std::wstring HashFile(const std::wstring& path){
    ...
    if(BCryptGetProperty(alg,BCRYPT_OBJECT_LENGTH,(PUCHAR)&objLen,sizeof(objLen),&cb,0)!=0||
       BCryptGetProperty(alg,BCRYPT_HASH_LENGTH,(PUCHAR)&hashLen,sizeof(hashLen),&cb,0)!=0){
        BCryptCloseAlgorithmProvider(alg,0);return L"";
    }
    ...
    if(!f){BCryptDestroyHash(hash);BCryptCloseAlgorithmProvider(alg,0);return L"";}
    char buf[1<<16];
    while(f){
        f.read(buf,sizeof(buf));auto n=f.gcount();
        if(n>0&&BCryptHashData(hash,(PUCHAR)buf,(ULONG)n,0)!=0){BCryptDestroyHash(hash);BCryptCloseAlgorithmProvider(alg,0);return L"";}
    }
    if(BCryptFinishHash(hash,digest.data(),hashLen,0)!=0){BCryptDestroyHash(hash);BCryptCloseAlgorithmProvider(alg,0);return L"";}
...
EngineTrust VerifyEngine(const std::wstring& appDir,const std::wstring& relativePath,const std::wstring& logicalName){
    ...
    if(relativePath.empty()||rel.is_absolute()||rel.has_root_name()||rel.has_root_directory()){
        t.reason=L"Engine path must be relative to the LapSure application root.";return t;
    }
    for(const auto& part:rel){
        if(part==L".."){t.reason=L"Engine path traversal outside the LapSure application root is blocked.";return t;}
    }

    std::error_code ec;
    const auto root=fs::weakly_canonical(fs::absolute(fs::path(appDir),ec),ec);
    if(ec||root.empty()){t.reason=L"Application root canonicalization failed.";return t;}
    const auto candidate=fs::weakly_canonical(root/rel,ec);
    if(ec||candidate.empty()){t.reason=L"Engine path canonicalization failed.";return t;}
    if(!IsWithinRoot(root,candidate)){t.reason=L"Engine resolved outside the LapSure application root.";return t;}
    if(!fs::is_regular_file(candidate,ec)||ec){t.reason=L"Engine file not found or is not a regular file.";return t;}
    t.fileExists=true;

    t.sha256=HashFile(candidate.wstring());
    if(t.sha256.empty()){t.reason=L"SHA-256 calculation failed.";return t;}
    ...
    while(std::getline(f,line)){
        ...
        const auto p=line.find(L'=');if(p==std::wstring::npos)continue;
        auto name=line.substr(0,p),expected=line.substr(p+1);
        Trim(name);Trim(expected);
    ...
    if(!IsSha256Hex(t.expectedSha256)){t.reason=L"Allowlist SHA-256 is not configured or invalid.";return t;}
